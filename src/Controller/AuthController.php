...
class AuthController extends AbstractController
{
    ...
    public function signup(Request $request, ?string $role = null): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Les admins inscrits : le premier est auto-activ�, les autres restent SUSPENDU
            if ($user instanceof Admin) {
                $existingAdmins = $this->entityManager->getRepository(Admin::class)->findAll();
                if (empty($existingAdmins)) {
                    // First admin - auto-activate
                    $user->setStatut(StatutCompte::ACTIF);
                } else {
                    // Subsequent admins need validation
        ...
        if ($user instanceof Patient) {
            ...
            if (isset($data['dateNaissance']) && !empty($data['dateNaissance'])) {
                try {
                    ...
                    // Si la date est invalide, on la saute
                }
            }
